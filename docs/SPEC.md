...
- Wraps everything in `<div data-mast-root data-mast-theme={theme}>` so the library's CSS custom properties resolve across the whole sidebar (header chrome, dialogs, `PlanConfirmationWidget`) — not just the library children. `<AgentProvider>` is configured with `disableRoot` in `AgentProviderShim` to avoid a second auto-rendered root: a layout-bare `<div data-mast-root>` from the library would break the flex chain that `<MessageList>`'s virtualizer needs to scroll. Token mapping is provided by the upstream `@mast-ai/react-ui/themes/tailwind-shadcn.css` preset (imported in `src/main.tsx`) so library components inherit the app theme — including dark mode toggled via the `.dark` class on `<html>`.
