...
- **Custom Skills (Sub-Agents):** Users can define custom specialized agents by providing:
  ...
  - The main agent's system prompt is dynamically injected with only the *names* and *descriptions* so it knows when to delegate. The *instructions* are only loaded and used by the sub-agent when `delegate_to_skill` is invoked.
